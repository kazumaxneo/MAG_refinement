...
def process_bin(ref, args, bin_threads):
    try:
        ...
        # --- Short reads mapping only (Bowtie2) ---
        ...
        # compress and move
        run_cmd(["pigz", f"-p{max(2,bin_threads//2)}",
                 f"map_{base_name}_R1.fastq", f"map_{base_name}_R2.fastq"], quiet=True)
        ...
        print(f"[INFO] Extracted short reads for {base_name}")

        # --- extract_sr_only オプションがあればここで終了 ---
        if args.extract_sr_only:
            return base_name

        # --- 以下は従来の HiFi マッピング + アセンブリ処理 ---
        ...
        if os.path.exists(f"{flye_outdir}/assembly.fasta"):
            run_cmd(["cp", f"{flye_outdir}/assembly.fasta",
                     f"{args.refined_bin_dir}/{base_name}_Flye.fa"])
        else:
        ...
        # SPAdes
        spades_cmd = [
            "spades.py", "-1", f"{args.sr_dir}/map_{base_name}_R1.fastq.gz",
            "-2", f"{args.sr_dir}/map_{base_name}_R2.fastq.gz",
            "--pacbio", hifi_output, "-o", f"{base_name}_spades-hybrid",
            "-t", str(max(1, bin_threads//2))
        ]
        with open(log_file, "a") as log:
            run_cmd(spades_cmd, stdout=log, stderr=subprocess.STDOUT)
        if os.path.exists(f"{base_name}_spades-hybrid/scaffolds.fasta"):
            run_cmd(["cp", f"{base_name}_spades-hybrid/scaffolds.fasta",
                     f"{args.refined_bin_dir}/{base_name}_SPAdes-hybrid.fa"])
        shutil.rmtree(f"{base_name}_spades-hybrid", ignore_errors=True)

        # Unicycler
        unicycler_cmd = [
            "unicycler", "-1", f"{args.sr_dir}/map_{base_name}_R1.fastq.gz",
            "-2", f"{args.sr_dir}/map_{base_name}_R2.fastq.gz",
            "-l", hifi_output, "-o", f"{base_name}_unicycler-hybrid",
            "-t", str(max(1, bin_threads//2))
        ]
        with open(log_file, "a") as log:
            run_cmd(unicycler_cmd, stdout=log, stderr=subprocess.STDOUT)
        if os.path.exists(f"{base_name}_unicycler-hybrid/assembly.fasta"):
            run_cmd(["cp", f"{base_name}_unicycler-hybrid/assembly.fasta",
                     f"{args.refined_bin_dir}/{base_name}_Unicycler-hybrid.fa"])
...
def main():
    ...
    parser.add_argument("--extract_sr_only", action="store_true",
                        help="Extract short reads per bin and stop after saving them.")
    ...
    # Seqkit stats (only if not extract_sr_only)
    if not args.extract_sr_only:
        try:
            run_cmd(["seqkit", "stats", "-T"] + bin_files, quiet=True,
                    stdout=open("bin_original_stats.tsv", "w"))
            refined_files = [os.path.join(args.refined_bin_dir, f)
                             for f in os.listdir(args.refined_bin_dir)
                             if f.endswith(".fa") or f.endswith(".fna")]
            if refined_files:
                run_cmd(["seqkit", "stats", "-T"] + refined_files, quiet=True,
                        stdout=open("bin_refined_stats.tsv", "w"))
